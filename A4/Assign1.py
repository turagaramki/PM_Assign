...
#         self.theta=np.arctan2((self.end.imag-self.start.imag),(self.end.real-self.start.real))
    ...
    def vortexpanel_coeff(self, field):
        ...
        if self.pnl_type == 0:
            ...
            # *np.exp(-1j*self.theta)
            complex_coef = 1j/(2*np.pi) * \
                np.log((zprime-self.l)/zprime)*self.t.conjugate()
            self.coeff = complex_coef.conjugate()
        else:
            zprime = (field-self.start)*self.t.conjugate()
            complex_coeff1 = 1j*(2*np.pi)*((zprime/self.l-1) *
                                           np.log((zprime-self.l)/zprime)+1.0)*self.t.conjugate()
            complex_coeff2 = -1j * (2*np.pi)*((zprime/self.l) *
                                              np.log((zprime-self.l)/zprime)+1.0)*self.t.conjugate()
            self.coeff[0] = complex_coeff1.conjugate()
            self.coeff[1] = complex_coeff2.conjugate()
    ...
    def vortexpanel_vel(self, field, strength):
        ...
        zprime = (field-self.start)*self.t.conjugate()
        # *np.exp(-1j*self.theta)
        complex_vel = 1j*strength / \
            (2*np.pi)*np.log((zprime-self.l)/zprime)*self.t.conjugate()
        self.complex_vel = complex_vel.conjugate()
        return complex_vel.conjugate()

    def panel_normal(self, origin):
        self.normal = (self.contol_point-origin) / \
            np.abs(self.contol_point-origin)

    def draw(self):
        plt.plot([self.start.real, self.end.real], [
                 self.start.imag, self.end.imag], linewidth=6, c='#0000ff')
...
def single_panel(pnl_type=0, angle=0):
    plt.close('all')
    print pnl_type
...
#     angle=25
    ...
    end = complex(start.real+np.cos(angle*np.pi/180),
                  start.imag+np.sin(angle*np.pi/180))

    c = 0.5*(start+end)

    z = create_mesh(x_low=c.real-2, x_up=c.real+2, n_x=100,
                    y_low=c.imag-1, y_up=c.imag+1, n_y=100)
...
def assemble_coeff_mat(Ps):
    ...
    for i in range(no_panels):
        for j in range(no_panels):
            if not Ps[j].type:
                cm[i, j] = complex_mult(Ps[j].vortexpanel_coeff(
                    Ps[i].contol_point), Ps[i].normal)
            else:
                print('Here')
                cm[i, j] = complex_mult(Ps[j].vortexpanel_coeff(
                    Ps[i].contol_point), Ps[i].normal)
...
def get_velocity(Ps, Vinf, vortex_pos, vortex_str, cm):
    bm = assemble_b_mat(Ps, Vinf, vortex_pos=vortex_pos,
                        vortex_str=vortex_str).real
    bm[-1] = 0.0
...
def problem1(n, r):
    ...
    z = create_mesh(x_low=-2+origin.real-r, x_up=2+origin.real+r,
                    n_x=100, y_low=-1+origin.imag-r, y_up=1+origin.imag+r, n_y=100)
...
#     gamma_mat=np.linalg.solve(cm,bm)
    ...
    for i in range(no_panels):
        Ps[i].draw()
    circle = plt.Circle((origin.real, origin.imag),
                        radius=radius, color='#0000ff', alpha=0.5)
    plt.gca().add_patch(circle)
    plt.xlabel('X', fontsize=18)
    plt.ylabel('Y', fontsize=18)
    plt.title('Stream lines with %d Constant vorticity panels' %
              no_panels, fontsize=18)
    plt.xticks(fontsize=12)
    plt.yticks(fontsize=12)
    plt.grid()
...
#     z=
    result = rk2_integrate(vortex_pos, vortex_pos,
                           vortex_str, Vinf, dt, tf, Ps, cm)
    plt.axis('equal')
    plt.plot(result.real, result.imag)
    plt.xlabel('X', fontsize=18)
    plt.ylabel('Y', fontsize=18)
    plt.title('Vortex trajectory with %d Constant vorticity panels' %
              no_panels, fontsize=18)
#     plt.legend(loc=1,fontsize=10)
    plt.xticks(fontsize=12)
    plt.yticks(fontsize=12)
...
# def source_vel(z, source_pos, source_str):
    Complex_vel = vortex_vel(z, vortex_b_loc, strength) + \
        vortex_vel(z, vortex_c_loc, -strength)
...
if __name__ == '__main__':
    single_panel(angle=55, pnl_type=1)
#     problem1(35,1.0)
